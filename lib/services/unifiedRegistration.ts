...
class UnifiedRegistrationService {
  ...
  async registerParticipant(data: CompleteRegistrationData): Promise<RegistrationResult> {
    try {
      ...
      const registrationData = {
        ...
        payment_screenshot_path: uploadResult.url!,
        payment_amount: tierAmount,
        status: 'pending',
  ...
  /**
   * Generate a unique order ID for registration
   */
  generateOrderId(): string {
    const timestamp = Date.now()
    const random = Math.random().toString(36).substring(2, 8)
    return `SPANDAN_${timestamp}_${random}`.toUpperCase()
  }
}
