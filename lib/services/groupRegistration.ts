...
export class GroupRegistrationService {
  ...
  /**
   * Calculate total amount for group registration
   */
  static calculateTotalAmount(members: MemberFormData[]): number {
    return members.reduce((total, member) => {
      return total + TIER_PRICES[member.tier];
    }, 0);
  }
   ...
   */
  static async submitRegistration(formData: RegistrationFormData): Promise<{
    success: boolean;
    groupId?: string;
    delegateIds?: string[];
    error?: string;
    ...
    try {
      ...
      const totalAmount = this.calculateTotalAmount(formData.members);
      ...
      const memberInserts = formData.members.map((member, index) => ({
        ...
        tier: member.tier,
        tier_amount: TIER_PRICES[member.tier],
      ...
      const delegateIds = membersData.map(m => m.delegate_user_id);
  ...
  static async getRegistrationStatus(
    ...
    try {
      // Try to find by email first (contact email or member email)
   ...
   */
  static validateRegistrationData(formData: RegistrationFormData): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (!formData.members || formData.members.length === 0) {
      ...
      formData.members.forEach((member, index) => {
        ...
        if (!member.tier || !Object.keys(TIER_PRICES).includes(member.tier)) {
          errors.push(`${memberPrefix} Valid tier selection is required`);
        }
      });
